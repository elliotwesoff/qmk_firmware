#include QMK_KEYBOARD_H
#include <stdbool.h>
#include "secrets.h"

enum custom_keycodes {
  M1, M2, M3, M4, M5, M6, M7, M8, M9, M10
};

bool capsLock = false;

const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  [0] = LAYOUT_80_with_macro(
    XXXXXXX,                          KC_ESC,  KC_F1,   KC_F2,   KC_F3,   KC_F4,   KC_F5,   KC_F6,   KC_F7,   KC_F8,   KC_F9,   KC_F10,  KC_F11,  KC_F12,           LGUI(KC_SPC), KC_MUTE,
    LCTL(LSFT(KC_TAB)), LCTL(KC_TAB), KC_GRV,  KC_1,    KC_2,    KC_3,    KC_4,    KC_5,    KC_6,    KC_7,    KC_8,    KC_9,    KC_0,    KC_MINS, KC_EQL,  _______, KC_BSPC,      KC_HOME,
    LGUI(KC_7), LGUI(KC_Z),           KC_TAB,  KC_Q,    KC_W,    KC_E,    KC_R,    KC_T,    KC_Y,    KC_U,    KC_I,    KC_O,    KC_P,    KC_LBRC, KC_RBRC,          KC_BSLS,      KC_END,
    LGUI(KC_5), LGUI(KC_6),           KC_LCTL, KC_A,    KC_S,    KC_D,    KC_F,    KC_G,    KC_H,    KC_J,    KC_K,    KC_L,    KC_SCLN, KC_QUOT,                   KC_ENT,       KC_PGUP,
    LGUI(KC_3), LGUI(KC_4),           KC_LSFT,          KC_Z,    KC_X,    KC_C,    KC_V,    KC_B,    KC_N,    KC_M,    KC_COMM, KC_DOT,  KC_SLSH, KC_RSFT,          KC_UP,        KC_PGDN,
    LGUI(KC_1), LGUI(KC_2),           MO(1),   KC_LALT, KC_LGUI, KC_LGUI, _______, KC_LALT,          XXXXXXX, KC_SPC,  KC_RGUI, KC_RALT, KC_RCTL, KC_LEFT,          KC_DOWN,      KC_RGHT
  ),
  [1] = LAYOUT_80_with_macro(
    _______,    QK_BOOT, _______, TG(2),   TG(3),   _______, _______, _______, _______, _______, _______,  _______, _______, _______,          KC_PSCR,   _______,
    M9, M10,    _______, RM_PREV, RM_NEXT, _______, _______, _______, _______, _______, _______, _______,  _______, _______, _______, _______, KC_DELETE, _______,
    M7, M8,     _______, _______, _______, _______, _______, _______, _______, _______, _______, _______,  _______, _______, _______, _______, _______,
    M5, M6,     KC_CAPS, _______, _______, _______, _______, _______, KC_LEFT, KC_DOWN, KC_UP,   KC_RIGHT, _______, _______,          _______, _______,
    M3, M4,     _______,          _______, _______, _______, _______, _______, _______, _______, KC_HOME,  KC_END,  _______, _______, KC_VOLU, _______,
    M1, M2,     _______, TG(2),   TG(3),   _______, _______, KC_TRNS,          _______, _______, _______,  _______, _______, _______, KC_VOLD, _______
  ),
  [2] = LAYOUT_80_with_macro(
    _______,            _______,    _______,    _______,    _______,    _______,    _______,    _______,    _______,    _______,    _______,    _______, _______, _______,          _______, _______,
    _______, _______,   _______,    _______,    _______,    _______,    _______,    _______,    _______,    _______,    _______,    _______,    _______, _______, _______, _______, _______, _______,
    _______, _______,   _______,    _______,    _______,    _______,    _______,    _______,    _______,    _______,    _______,    _______,    _______, _______, _______, _______, _______,
    _______, _______,   _______,    _______,    _______,    _______,    _______,    _______,    _______,    _______,    _______,    _______,    _______, _______,          _______, _______,
    _______, _______,   _______,    _______,    _______,    _______,    _______,    _______,    _______,    _______,    _______,    _______,    _______, _______, _______, _______,
    _______, _______,   _______,    _______,    KC_LGUI,    KC_SPC,     KC_TRNS,    _______,                _______,    _______,    _______,    _______, _______, _______, _______, _______
  ),
  [3] = LAYOUT_80_with_macro(
    _______,            _______,    _______,    _______,    _______,    _______,    _______,    _______,    _______,    _______,    _______,    _______, _______, _______,          _______, _______,
    _______, _______,   _______,    LGUI(KC_1), LGUI(KC_2), LGUI(KC_3), LGUI(KC_4), LGUI(KC_5), LGUI(KC_6), LGUI(KC_7), LGUI(KC_8), LGUI(KC_9), _______, _______, _______, _______, _______, _______,
    _______, _______,   _______,    _______,    _______,    _______,    _______,    _______,    _______,    _______,    _______,    _______,    _______, _______, _______, _______, _______,
    _______, _______,   _______,    _______,    _______,    _______,    _______,    _______,    _______,    _______,    _______,    _______,    _______, _______,          _______, _______,
    _______, _______,   _______,    _______,    _______,    _______,    _______,    _______,    _______,    _______,    _______,    _______,    _______, _______, _______, _______,
    _______, _______,   _______,    _______,    _______,    _______,    KC_TRNS,    _______,                _______,    _______,    _______,    _______, _______, _______, _______, _______
  )
};

bool process_record_user(uint16_t keycode, keyrecord_t *record) {
  switch (keycode) {
    case M1:
      if (record->event.pressed) {
        send_string_with_delay(PPTUSER, 10);
        tap_code_delay(KC_TAB, 20);
        send_string_with_delay(PPTPASS, 10);
        tap_code_delay(KC_ENTER, 20);
      }
      break;
    case M4:
      if (record->event.pressed) {
        send_string(ONEPASS);
      }
      break;
    default:
      break;
  }
  return true;
}

bool rgb_matrix_indicators_advanced_user(uint8_t led_min, uint8_t led_max) {
  // notes:
  // * led_min + 1, 4 lights up an underglow light directly beneath F1... ???
  // * this function gets called repeatedly and very fast.

  if (layer_state_is(1)) {
    rgb_matrix_set_color(led_min + 2, RGB_YELLOW); // F1
  }

  if (layer_state_is(2)) {
    rgb_matrix_set_color(led_min + 3, RGB_BLUE); // F2
  }

  if (layer_state_is(3)) {
    rgb_matrix_set_color(led_min + 5, RGB_GREEN); // F3
  }

  if (host_keyboard_led_state().caps_lock) {
    rgb_matrix_set_color(led_min, RGB_RED); // ESC
  }

  return false;
}

bool encoder_update_user(uint8_t index, bool clockwise) {
    if (index == 0) {
        if (clockwise) {
            if (layer_state_is(2)) {
                tap_code16(LCTL(LGUI(KC_LEFT)));
            } else {
                tap_code(QK_MOUSE_WHEEL_UP);
            }
        } else {
            if (layer_state_is(2)) {
                tap_code16(LCTL(LGUI(KC_RIGHT)));
            } else {
                tap_code(QK_MOUSE_WHEEL_DOWN);
            }
        }
    } else if (index == 1) {
        if (clockwise) {
            tap_code(KC_VOLD);
        } else {
            tap_code(KC_VOLU);
        }
    }
    return false;
}
